...
class DefaultChunkManagerTest extends AesKeyAwareTest {
    ...
    void testGetChunk() throws Exception {
        ...
        final var part = new Part(chunkIndex, chunkIndex.chunks().get(0), 1);
        assertThat(chunkManager.chunksContent(OBJECT_KEY_PATH, manifest, part)).hasContent("0123456789");
    ...
    void testGetChunkWithEncryption() throws Exception {
        ...
        assertThat(chunkManager.chunksContent(OBJECT_KEY_PATH, manifest, part)).hasBinaryContent(TEST_CHUNK_CONTENT);
        verify(storage).fetch(OBJECT_KEY_PATH, chunkIndex.chunks().get(0).range());
    }
